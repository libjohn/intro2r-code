...
**Use THIS**.  This new package works well and allows for the selection of columns.  https://vroom.r-lib.org/reference/vroom.html

```{r}
v_data <- vroom("data/2008.zip", col_select = starts_with("D"))
...
```{r message=FALSE, warning=FALSE}
png("images/time_vs_dist.png", width=1024, height=800)
ggplot(flights_small, aes(y=AirTime,x=Distance)) +
  geom_point(alpha=0.01, size=0.5)

ggsave("images/time_vs_dist_ggsave.png")
```


![](images/time_vs_dist_ggsave.png)
